...
//   new G4SubtractionSolid("RsbsubtconeminusPL", solidRsbmp, solidFullconemp);
  ...
  G4double RsbsubtconeminusPL_rmina = (6.0+13.0+13.0)*cm, RsbsubtconeminusPL_rmaxa = (70./2)*cm;
  G4double RsbsubtconeminusPL_rminb = (6.0)*cm, RsbsubtconeminusPL_rmaxb = (70./2)*cm;
  G4double RsbsubtconeminusPL_hz = ((0.5)*(0.5+36.5+0.5+1.0+0.5))*cm;
  G4double RsbsubtconeminusPL_phimin = 0.*deg, RsbsubtconeminusPL_phimax = 360.*deg;
  G4Cons* RsbsubtconeminusPL =    
    new G4Cons("RsbsubtconeminusPL", 
    RsbsubtconeminusPL_rmina, RsbsubtconeminusPL_rmaxa, RsbsubtconeminusPL_rminb, RsbsubtconeminusPL_rmaxb, RsbsubtconeminusPL_hz,
    RsbsubtconeminusPL_phimin, RsbsubtconeminusPL_phimax);
  ...
  G4Material* material_Teflon = nist->FindOrBuildMaterial("G4_TEFLON"); //Teflon 
  G4Material* material_GRAPHITE = nist->FindOrBuildMaterial("G4_GRAPHITE"); //  GRAPHITE
  

//  G4ThreeVector pos_rside_reflector = G4ThreeVector(0, 0, ((0.5+36.5+0.5+1.0+0.5)/2)*cm);
             
  G4double wrap_rmina = (6.0+13.0)*cm, wrap_rmaxa = (6.0+13.0+13.0)*cm;
  G4double wrap_rminb = (6.0)*cm, wrap_rmaxb = (6.0)*cm;
  G4double wrap_hz = ((0.5+36.5+0.5+1.0+0.5)/2)*cm;
  G4double wrap_phimin = 0.*deg, wrap_phimax = 360.*deg;
  G4Cons* WrappingLayer =    
    new G4Cons("WrappingLayer", 
    wrap_rmina, wrap_rmaxa, wrap_rminb, wrap_rmaxb, wrap_hz,
    wrap_phimin, wrap_phimax);

//  G4SubtractionSolid* WrappingLayer =
//   new G4SubtractionSolid("WrappingLayer", solidwrap, solidFullconemp);


  G4LogicalVolume* logicWrappingLayer =                         
    new G4LogicalVolume(WrappingLayer,         //its solid
                    //    vacuum,          //its material vacuum
                        AlF3_material,          //its material AlF3
                    //    material_Teflon,   // material Teflon
                    //    material_GRAPHITE, // material GRAPHITE
                        "WrappingLayer");               //its name
  //logicWrappingLayer->SetVisAttributes(new G4VisAttributes(G4Colour(204/255.,0/255.,204/255.)));
              
  new G4PVPlacement(0,                       //no rotation
                    pos_rside_reflector,                    //at position
                    logicWrappingLayer,             //its logical volume
                    "WrappingLayer",                //its name
                    logicWorld,                //its mother  volume
                    false,                   //no boolean operation
                    0,                       //copy number
